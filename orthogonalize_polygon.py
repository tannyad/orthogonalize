...
def get_verticies(layer):
    n_vertices=[]
    multipolygon = gpd.GeoDataFrame({
        'geometry': []
    })
    multipolygon.set_geometry('geometry')
    for i, row in layer.iterrows():
        build = layer.loc[i, 'geometry']
        if build.geom_type == 'MultiPolygon':
            multipolygon['geometry'][0] = build
            multipolygon.loc[0, 'geometry'] = build
            exploded = multipolygon.explode(index_parts=False)
            n = 0
            for part in exploded.geometry:
    ...
    for i in range(0, len(layer)):
        ...
        if build.geom_type == 'MultiPolygon':
            ...
            for poly in exploded.geometry:
                ...
                multipolygons.append(buildOrtho)
            layer.loc[i, 'geometry'] = gpd.GeoSeries(MultiPolygon(multipolygons)).values # Workaround for Pandas/Geopandas bug
        elif build.geom_type != 'MultiPolygon':    # Polygons
            buildOrtho = orthogonalize_polygon(build)
            layer.loc[i, 'geometry'] = buildOrtho
    return layer


def main():
    # Считать файл.
    buildings = gpd.read_file('s.shp', encoding = 'UTF-8')
    # Выполнить процедуру "Упрощение" (начальное упрощение чтобы извабиться от пикселбных границ).
    simple_buildings = buildings
    simple_buildings.geometry = simple_buildings.simplify(0.000005)
    # Убрать все дыры.
    for i in range(0, len(simple_buildings)):
        build = simple_buildings.loc[i, 'geometry']
        list_parts = []
        if build.type == 'MultiPolygon': # Multipolygons
            for polygon in build.geoms:
                temp_pol = Polygon(polygon.exterior.coords)
                list_parts.append(temp_pol)
            buildFull = MultiPolygon(list_parts)
            simple_buildings.loc[i, 'geometry'] = buildFull
        else:
            buildFull = Polygon(build.exterior.coords)
            simple_buildings.loc[i, 'geometry'] = buildFull
    # Добавить столбец с количеством вершин у полигона.
    buildings_with_vert = get_verticies(simple_buildings)
    # В зависимости от количества вершин выполнить "Упрощение" с различными коэфициентами.
    for i in range(0, len(buildings_with_vert)):
        num_vert = buildings_with_vert.loc[i, 'n_vertices']
        build = buildings.loc[i, 'geometry']
        if num_vert < 10:
            buildSimple = build.simplify(0.000025)
        elif num_vert >=10 and num_vert <30:
            buildSimple = build.simplify(0.000003)
        elif num_vert >=30 and num_vert <50:
            buildSimple = build.simplify(0.0000085)
        elif num_vert >=50 and num_vert <70:
            buildSimple = build.simplify(0.0000125)
        elif num_vert >=70 and num_vert <90:
            buildSimple = build.simplify(0.0000225)
        elif num_vert >=91 and num_vert <100:
            buildSimple = build.simplify(0.00003)
        elif num_vert >= 100:
            buildSimple = build.simplify(0.0000555)
        buildings.loc[i, 'geometry'] = buildSimple
    # Спрямить углы углы.
    orto_build = make_angle(buildings)
    # Пофиксить невалидную геометрию.
    orto_build['geometry'] = orto_build.buffer(0)
    # Убрать все дыры.
    for i in range(0, len(orto_build)):
        build = orto_build.loc[i, 'geometry']
        list_parts = []
        if build.type == 'MultiPolygon': # Multipolygons
            for polygon in build.geoms:
                temp_pol = Polygon(polygon.exterior.coords)
                list_parts.append(temp_pol)
            buildFull = MultiPolygon(list_parts)
            orto_build.loc[i, 'geometry'] = buildFull
        else:
            buildFull = Polygon(build.exterior.coords)
            orto_build.loc[i, 'geometry'] = buildFull
    # Перевести в метрическую СК.
    orto_build = orto_build.to_crs(3857)
    # Удалить все полигоны с мелкой площадью.
    orto_build = orto_build[orto_build.geometry.area > 30]
    # Вернуть в СК 4326.
    orto_build = orto_build.to_crs(4326)
    orto_build.to_file('outFile.shp', encoding = 'UTF-8')
